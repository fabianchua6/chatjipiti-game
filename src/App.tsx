...
type AgentRun = { prompt: string; started: number; status: 'working' | 'complete' | 'stopped' };
const games = [
...
function currentPage(): Page { const hash = location.hash.slice(1) as Page; return validPages.includes(hash) ? hash : 'chat'; }
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      setElapsed(seconds);
      if (seconds >= 75) { setRun(current => current ? { ...current, status: 'complete' } : null); clearInterval(timer); }
    }, 500);
  ...
  function send(prompt = composer) {
    ...
    setRun({ prompt: prompt.trim().slice(0, 1200), started: performance.now(), status: 'working' });
    setElapsed(0); setComposer(''); setDismissed(false); navigate('chat');
  }
  function newChat() { setRun(null); setElapsed(0); setComposer(''); setDismissed(false); navigate('chat'); }
  function toggleMute() {
  ...
  const agentPhase = elapsed < 15 ? 'Understanding your request' : elapsed < 35 ? 'Exploring a few approaches' : elapsed < 55 ? 'Putting the pieces together' : 'Checking the details';
  ...
  return <div className="chat-shell">
    ...
    <aside ref={sidebar} id="sidebar" className={`sidebar ${drawer ? 'is-open' : ''}`} aria-label="Main navigation" {...(mobile && drawer ? { role: 'dialog', 'aria-modal': true } : {})} onKeyDown={event => {
      ...
      <div className="sidebar-brand"><button className="brand-mark icon-button" onClick={newChat} aria-label="ChatJiPiTi home"><Icon name="spark"/></button><span>ChatJiPiTi</span>{mobile && <button className="icon-button" onClick={() => { setDrawer(false); menu.current?.focus(); }} aria-label="Close navigation"><Icon name="close"/></button>}</div>
    ...
    <div className="workspace" inert={mobile && drawer}>
      <header className="topbar"><div><button ref={menu} className="icon-button mobile-menu" aria-label="Open sidebar" aria-expanded={drawer} aria-controls="sidebar" onClick={() => setDrawer(true)}><Icon name="menu"/></button><button className="topbar-title" onClick={() => navigate('chat')}>ChatJiPiTi <span>Astra 6</span></button><span className="demo-pill">Demo</span></div><button className="icon-button" aria-label={muted ? 'Enable game sounds' : 'Mute game sounds'} aria-pressed={!muted} onClick={toggleMute}><Icon name={muted ? 'mute' : 'sound'}/></button></header>
      {run && run.status !== 'stopped' && page !== 'chat' && <div className="agent-strip">{run.status === 'working' ? <span className="thinking-dot"/> : <Icon name="check"/>}<span>{run.status === 'working' ? 'Your agent is working' : 'Your agent is ready'} · {elapsed}s</span><button onClick={() => navigate('chat')}>View chat <Icon name="chevron"/></button></div>}
      <main ref={content} className="content-scroll" tabIndex={-1}>
        {page === 'chat' && <div className={`chat-page ${run ? 'has-conversation' : ''}`}>
          {!run ? <div className="chat-welcome"><div className="welcome-mark"><Icon name="spark"/></div><h1>What are we building today?</h1><p>The big idea is yours. The waiting time is game time.</p></div> : <div className="conversation"><div className="user-message">{run.prompt}</div><div className="assistant-message"><span className="assistant-avatar"><Icon name="spark"/></span><div><p className="assistant-name">ChatJiPiTi</p>{run.status === 'working' ? <><div className="thinking-label"><span className="thinking-dot"/>{agentPhase}<span>{elapsed}s</span></div><p>I’m on it. This might take a little thinking.</p><div className="game-invitation"><span className="invitation-art"><Icon name="games"/></span><div><h2>Fancy a game?</h2><p>I’ll keep working. You go beat your best.</p><button onClick={() => navigate('games')}>Let’s play <Icon name="chevron"/></button></div></div><p className="mock-note">Simulated agent run · finishes in about 75 seconds.</p></> : run.status === 'stopped' ? <p>Stopped. Ready when you are.</p> : <><p>Your demo task is ready. Here’s the direction I’d take:</p><ul className="demo-answer"><li>Start with one clear outcome and a small first version.</li><li>Make the main interaction feel good on a phone.</li><li>Test the whole experience, then share what you made.</li></ul><p className="mock-note">Scripted demo response. No model request was made.</p><button className="secondary" onClick={() => navigate('games')}>Back to your games <Icon name="games"/></button></>}</div></div></div>}
          <div className="composer-area"><form className="composer" onSubmit={event => { event.preventDefault(); send(); }}><label className="sr-only" htmlFor="chat-prompt">Message ChatJiPiTi</label><textarea id="chat-prompt" rows={2} value={composer} onChange={event => setComposer(event.target.value)} placeholder="Ask anything, then play a little." maxLength={1200} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey && !event.nativeEvent.isComposing) { event.preventDefault(); send(); } }}/><div className="composer-controls"><span><Icon name="spark"/>Astra · thinking</span>{run?.status === 'working' ? <button type="button" className="send-button" aria-label="Stop demo agent" onClick={() => setRun(current => current ? { ...current, status: 'stopped' } : null)}><Icon name="stop"/></button> : <button type="submit" className="send-button" aria-label="Send message" disabled={!composer.trim()}><Icon name="arrow"/></button>}</div></form>{!run && <div className="prompt-suggestions">{suggestions.map(prompt => <button key={prompt} onClick={() => send(prompt)}>{prompt}</button>)}</div>}<p className="composer-note">A playful ChatGPT-style demo. No real chats, sign-in or account limits are connected.</p></div>
        </div>}
        ...
        {activeGame && <div className="game-page"><div className="game-toolbar"><button className="text-button" onClick={() => navigate('games')}><Icon name="back"/>All games</button><span>{mode === 'daily' ? 'Daily Challenge' : 'Free Play'}</span></div>{page === 'memory' && <MemoryGame key={mode} mode={mode}/>}{page === 'typing' && <TypingGame key={mode} mode={mode} muted={muted}/>}{page === 'circle' && <CircleGame key={mode} mode={mode} muted={muted}/>}</div>}
