...
export default function MatchCelebration({ sequence, combo }: { sequence: number; combo: number }) {
  ...
  return <div key={sequence} className="match-celebration" aria-hidden="true">
    ...
    <div className={`match-proclamation ${phrase.headline.length > 5 ? 'is-long-stack' : ''}`}><span>{phrase.eyebrow}</span><strong>{phrase.headline.map(line => <span key={line} className={line !== 'ABSOLUTELY' && line !== 'RIGHT' ? 'match-amplifier' : undefined} style={{ fontSize: `${Math.min(1, 10 / line.length) * 100}%` }}>{line}</span>)}</strong><small>{combo > 1 ? `${combo} PAIRS IN A ROW` : 'COSMICALLY CORRECT'}</small></div>
  </div>;
}
